...
void main() {
  group('Findex SQLite', () {
    ...
    //   final upsertResults = await SqliteFindex.indexAll(findexKey, label);
    ...
    //       stacktrace.toString(),
    //       contains(
    //           "test/findex/sqlite_findex.dart:277:7"), // When moving stuff inside this file, this assertion could fail because the line number change. Please set the line number to the line below containing :ExceptionLine
    //     );
    //   } finally {
    ...
    //     expect(
    //       e.toString(),
    //       "error deserializing findex key: try from slice error: could not convert slice to array",
    //     );
    //   }
    ...
    //         e.toString(),
    //         startsWith(
    //           "serialization: serialization error: crypto error: incorrect length for encrypted value: 32 bytes give, 114 bytes expected",
    //         ));
    //   } finally {
    ...
    //         e.toString(),
    //         startsWith(
    //             "serialization: serialization error: crypto error: incorrect length for encrypted value: 32 bytes give, 108 bytes expected"));
    //   } finally {
    ...
    //     FindexKey findexKey,
    //     Uint8List label,
    //     Map<IndexedValue, Set<Keyword>> indexedValuesAndKeywords,
    ...
    //   await SqliteFindex.upsert(findexKey, label, indexedValuesAndKeywords, {});
    ...
    // void checkResults(
    //     Map<Keyword, Set<Location>> searchResults, List<int> expectedIds,
    //     {String word = "John"}) {
    ...
    //   await insertNewIndexes(db1Path, findexKey, label, {
    //     IndexedValue.fromLocation(Location.fromNumber(1)): {
    //       Keyword.fromString("John")
    //     }
    //   });
    //   await insertNewIndexes(db2Path, findexKey, label, {
    //     IndexedValue.fromLocation(Location.fromNumber(2)): {
    //       Keyword.fromString("John")
    //     }
    //   });
    //   await insertNewIndexes(db3Path, findexKey, label, {
    ...
    //         findexKey.k, label, {Keyword.fromString("John")},
    //         entryTableNumber: 2);
    //   } catch (e) {
    //     expect(
    //       e.toString(),
    //       "'fetch' buffer too small: 1",
    //     );
    //   }
    ...
    //       findexKey.k, label, {Keyword.fromString("John")},
    //       entryTableNumber: 3);// TODO: something wrong here: value was 1 before and test succeeded
    ...
    //   searchResults = await SqliteFindexMultiEntryTables.search(
    //       findexKey.k, label, {Keyword.fromString("Marie")},
    //       entryTableNumber: 2);
    //   checkResults(searchResults, [1, 2], word: "Marie");

    //   searchResults = await SqliteFindexMultiEntryTables.search(findexKey.k,
